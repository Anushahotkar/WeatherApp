/* App.jsx - Responsive & Modern Version */
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-400 via-sky-300 to-blue-200 p-4 flex flex-col items-center">
      ...
      <header className="relative z-10 py-12 w-full max-w-6xl text-center">
        <div className="flex flex-col items-center space-y-4 px-4 sm:px-6 md:px-0">
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-4">
            <span className="text-6xl sm:text-7xl shadow-lg">☁️</span>
            <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold bg-gradient-to-r from-teal-500 via-sky-600 to-indigo-700 bg-clip-text text-transparent tracking-tighter drop-shadow-lg">
              WeatherFlow
            </h1>
            <span className="text-6xl sm:text-7xl shadow-lg">☀️</span>
          </div>
          <p className="text-base sm:text-lg md:text-2xl text-gray-700 font-light max-w-3xl leading-snug mt-2 px-2 sm:px-0">
      ...
      {/* Search Section */}
      <section className="relative z-10 w-full max-w-3xl mb-12 px-2 sm:px-0">
        <SearchBar onSearch={fetchWeather} loading={loading} />
      </section>

      {/* Main Content */}
      <main className="relative z-10 w-full max-w-6xl flex-1 pb-8 px-2 sm:px-0">
        
        {/* Loading State */}
        {loading && (
          <div className="text-center py-16 sm:py-20">
            <div className="inline-flex flex-col items-center space-y-5 bg-white p-6 sm:p-10 rounded-3xl shadow-2xl border border-blue-100/50">
              <div className="w-10 h-10 sm:w-12 sm:h-12 border-4 sm:border-6 border-sky-500 border-t-transparent rounded-full animate-spin"></div>
        ...
        {/* Initial Empty State */}
        {!loading && !weatherData && (
          <div className="text-center py-16 sm:py-20 px-2 sm:px-0">
            <div className="bg-white/90 backdrop-blur-md rounded-3xl p-8 sm:p-16 max-w-2xl mx-auto shadow-2xl border border-gray-200">
              <div className="text-6xl sm:text-8xl mb-6 transform hover:scale-105 transition-transform">🔍</div>
              <h3 className="text-3xl sm:text-4xl font-extrabold text-gray-800 mb-4 bg-gradient-to-r from-sky-500 to-indigo-500 bg-clip-text text-transparent">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 text-sm sm:text-base text-gray-600 font-medium">
      ...
      {/* Footer */}
      <footer className="relative z-10 mt-10 sm:mt-13 pb-4 text-center text-gray-500 text-sm border-t border-gray-200 pt-4 w-full max-w-6xl px-2 sm:px-0">
